...
// (src/figures/__fixtures__/measuredFigureType.ts), mirroring the convention
// tools/measure-battle-layout.mjs already established for M7/M12's
// src/battle/__fixtures__/measuredLayout.ts: the JSON is the human-readable
// source of truth for a given rig run, and the TS module is what a
// src/**/*.test.ts file actually imports (no resolveJsonModule; docs/ is
// outside tsconfig.app.json's "include": ["src"]). The TS module is
// GENERATED — regenerating it is this rig's job, not a builder's hand-edit,
// so the measurement chain cannot silently drift out of sync with the JSON
// the way a hand-copied module could.
//
// This script must NOT create or edit src/figures/layout.ts — that file does
// not exist yet; task A2 (a later, separate dispatch) creates it and takes
// its constants from this fixture, rounding UP for NODE_MIN_PX and
// MONO_CH_PX.
//
...
// only one figures fixture module, unlike OUT_DIR which is milestone-scoped.
...
async function main() {
  ...
  // ["src"]). This generated .ts module is the one A2/A3's tests import.
  // DO NOT HAND-EDIT — regenerate by re-running this script.
  const fixtureTsSrc = `// GENERATED by tools/measure-figure-type.mjs (S3 PR-A task A2a) — DO NOT
