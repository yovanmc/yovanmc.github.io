...
| 8 | Combined legal-max panel heights (see Context) computed in-session via pure seams over ALL FOUR bosses + hero; imposter-only arm independently reproduced by the pass-1 critic (exact match), Alert-Storm binding values re-derived after its BLOCKER | db4085a | — (Task A2's property test re-derives and pins these; until it lands the derivation in Context is the evidence) |
