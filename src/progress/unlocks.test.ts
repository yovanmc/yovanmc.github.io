...
import { SEED_UNLOCKED, UNLOCK_BY_BOSS, guardingBoss, isGateable, unlockedSlugs } from "./unlocks";
...
describe("guardingBoss", () => {
  it("returns the display name of the boss guarding each of the 4 mapped slugs", () => {
    for (const [bossId, slug] of Object.entries(UNLOCK_BY_BOSS)) {
      expect(guardingBoss(slug)).toBe(BOSS_NAMES[bossId]);
    }
  });

  it("is null for a seed-unlocked slug (mia never has a guarding boss)", () => {
    expect(guardingBoss("mia")).toBeNull();
  });

  it("is null for an unknown slug", () => {
    expect(guardingBoss("not-a-real-slug")).toBeNull();
  });
});
