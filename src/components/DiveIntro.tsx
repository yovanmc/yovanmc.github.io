...
import { playFullIntro } from "../landingCopy";
import {
  PAL,
  SPR_HERO,
  STAGE_W,
...
// Namespaced copies of the ambience keyframes in stage pixels. The site's
// global floatUp/twinkle/auroraA/B are vh-based and MUST NOT be reused here;
// these are the same animations ported 1:1 into stage pixels.
const DV_CSS = `
...
export function DiveIntro({ onHandoff, onDone, freezeAt }: DiveIntroProps) {
  ...
  if (reduced && !forceFull) {
    return <DiveIntroReduced onHandoff={onHandoff} onDone={onDone} onPlayFull={() => setForceFull(true)} />;
  }
...
export interface DiveIntroReducedProps {
  onHandoff: (target: IntroTarget) => void;
  onDone: () => void;
  /** Runs the full cinematic instead, from the top (opt-in per visit). */
  onPlayFull: () => void;
}
...
export function DiveIntroReduced({ onHandoff, onDone, onPlayFull }: DiveIntroReducedProps) {
  ...
  const station = useMemo(() => buildStationCanon("B"), []);

  useEffect(() => {
    if (heroRef.current) drawHero(heroRef.current, 0);
    const raf = requestAnimationFrame(() => setVisible(true));
    const timeline = reducedDiveTimeline();
    const handoffTimer = window.setTimeout(() => {
      if (!doneRef.current) cbRef.current.onHandoff("gate");
    }, timeline.handoffAtMs);
  ...
  return (
    <div
      style={REDUCED_STYLES.overlay}
      onClick={(e) => {
        if ((e.target as HTMLElement).closest("[data-dive-controls]")) return;
        skip();
      }}
    >
      <div style={{ ...REDUCED_STYLES.frame, opacity: visible ? 1 : 0 }}>
        <div
          aria-hidden
          style={{
            position: "absolute",
          ...
          dangerouslySetInnerHTML={{ __html: station }}
        />
        <canvas
          ref={heroRef}
          width={48}
...
function DiveIntroFull({ onHandoff, onDone, freezeAt }: DiveIntroProps) {
  ...
  useEffect(() => {
    ...
    // Bird + mote element pools, built once with namespaced classes.
    ...
    // Exposes the settled station rect so automated checks can assert framing.
    ...
    if (freezeRef.current !== undefined) {
      ...
      const loop = (now: number) => {
        ...
        if (t >= T_MENU + MENU_DUR) {
          // menu-rise beat = the gate's attach point, so settle here
          finish("gate", false);
          return;
        }
  ...
  // Two station copies sharing one document, with id-suffixed defs: a dim
  // copy and a lit copy, and the lit copy is clipped by the reveal circle.
