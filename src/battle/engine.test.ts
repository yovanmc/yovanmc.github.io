...
describe("victory", () => {
  ...
  it("rematch victory grants NO forge/rider/unlock EVENT (defeatedBosses gates them); maxHp stays the derived 110 the fight started at, never a second bump", () => {
...
describe("derived rider (carry-over is derived, not stored)", () => {
  ...
  it("scales past one boss (rider math is boss-count-generic)", () => {
...
describe("RUSH_ORDER / IMPLEMENTED_BOSSES", () => {
  it("rush order is the four bosses in fight order, and all four are implemented", () => {
...
describe("kit derivation", () => {
  ...
  it("cascade grants its rb unlock; a fake boss id is ignored, with no crash and no phantom kit growth", () => {
    expect(deriveKit(["cascade", "not-a-real-boss"])).toEqual(["attack", "ct", "pt", "debug", "rb"]);
  });

  it("imposter-syndrome defeated grants conv; imposter-syndrome NOT defeated does not, even alongside every other boss", () => {
  ...
  it("the reducer rejects an out-of-kit action type as invalid, without mutating turn/hero", () => {
    const s0 = initBattle({ seed: 42 });
...
describe("Multiplier core (the dealt/taken helpers, tested directly)", () => {
  ...
  it("CT alone (Conviction off) uses dealt ×1.5, taken ×0.75", () => {
  ...
  it("a rigged state with conviction:true feeds the same math (conviction is a plain state flag)", () => {
...
describe("Fan Out (base 8, hits all living targets, single reshuffle)", () => {
...
describe("the streamlined win line, end to end", () => {
...
describe("Cascade boot + dispatch", () => {
  ...
  it("initBattle boots boss: \"imposter-syndrome\" on request, opens in clones at full HP", () => {
  ...
  it("falls back to alert-storm for a fake boss id (never a crash path)", () => {
  ...
  it("Debug against the carrier marks it, deals 6 halved by the carrier shield (25 - floor(6/2) = 22), and starts a DoT", () => {
    ...
    expect(s.dots).toEqual([{ batId: 0, ticksLeft: 3, tick: 4 }]); // tick stamped at push time
  });
  ...
  describe("the engine-generated win line", () => {
    // CT@1, FO@2, FO@3, FO@4, Attack(carrier)@5, booted at the rematch-style
    // 110/12 start (defeatedBosses: ["alert-storm"]) so Fan Out is in kit.
    function winLine() {
    ...
    it("T5 Attack on the carrier finishes the fight: victory, forge=rollback, unlock=cascade, rider applied on top of the 52 HP the fight arrived at", () => {
      ...
      // 110 -> 52 HP over the fight, +10 rider on this first cascade win
  ...
  it("rematch victory (cascade already defeated) grants no forge/rider/unlock event", () => {
    // Rigged so only node 0 survives, at 1 HP, and a single attack ends it
    // immediately (victory short-circuits before any boss turn, so hero HP/MP
    // starting values don't matter here).
...
describe("Silent Failure engine wiring", () => {
  describe("targetability: pt/debug/fo invalid while vanished; attack is exempted and whiffs", () => {
    ...
    it("attack against an embodied boss deals ATTACK_DMG and grants +1 MP as usual", () => {
      const s0 = silentFailureState({ phase: "embodied", phaseTurnsLeft: 2 });
    ...
    it("a target id other than SF_TARGET_ID is invalid: findTarget's unmatched-id path returns undefined", () => {
      const s0 = silentFailureState({ phase: "embodied", phaseTurnsLeft: 2 });
  ...
  describe("Debug mark + DoT on the Silent Failure (the DoT ticks through the hidden phase)", () => {
    it("debug deals 6, marks it, and starts a DoT that has not ticked yet this turn", () => {
      ...
      expect(s1.dots).toEqual([{ batId: SF_TARGET_ID, ticksLeft: 3, tick: 4 }]); // tick stamped at push time
    });
  ...
  describe("Fan Out vs the Silent Failure (single target — degenerates to a plain hit, no reshuffle, no shield)", () => {
    ...
    it("lethal Fan Out (hp exactly hits 0) pushes batDown same as a direct hit", () => {
  ...
  describe("Rollback: heal 30 capped at maxHp, 3 MP, untargeted", () => {
  ...
  describe("victory: the forge event is root-cause and rc joins the kit", () => {
    it("defeating Silent Failure emits forge: root-cause, rider, and unlock, and grants the rc ability", () => {
      ...
      // base four + fo (alert-storm) + rb (cascade) + rc (silent-failure)
  ...
  describe("DoT-kill rule: a DoT tick that drops the boss to 0 while vanished fires victory and sets forceBodyForDeath", () => {
...
describe("Silent Failure boot + engine-generated win line", () => {
  ...
  it("hero arrives at 120/14 with Rollback in kit (defeatedBosses: alert-storm + cascade)", () => {
    const s = initBattle({ seed: 42, boss: "silent-failure", defeatedBosses: ["alert-storm", "cascade"] });
  ...
  describe("the engine-generated win line (fastest legal line: PT, PT, whiff, CT, PT, PT, 28+28+42+42 = exactly 140)", () => {
    ...
    it("reaches victory within the 6-11 hero-turn band, entering both phases, surviving at least one ambush, hero HP > 0", () => {
    ...
    it("emits victory + forge=root-cause + unlock=silent-failure, rider applied on top of the fight's ending HP", () => {
      ...
      // Defeating Silent Failure unlocks Root Cause.
      expect(deriveKit(s.defeatedBosses)).toContain("rc");
      // Conviction has no KIT_UNLOCKS entry, and this input never defeated
      // Imposter, so the kit gate keeps a live guard here.
...
describe("Root Cause (rc): 4 MP", () => {
  ...
  it("deals 22 unmarked, 33 vs a marked Imposter", () => {
    const s0 = imposterState();
    const s1 = battleReduce(s0, { type: "rc", target: 0 });
  ...
  it("always hits the real clone regardless of which slot is targeted, and never pops", () => {
  ...
  it("rips a VANISH phase back immediately: full hit lands and the phase ends early", () => {
...
describe("rc marked bonus + ignores-stealth (both player-reachable: beat Silent Failure, rc unlocks, rematch any boss)", () => {
  ...
  it("the ignore-stealth flag does not leak into attack/pt/debug against a vanished Silent Failure", () => {
    const s0 = silentFailureState({ phase: "vanished", phaseTurnsLeft: 2 });
...
describe("Conviction (conv): 5 MP, the hp*4 <= maxHp gate", () => {
  ...
  it("is castable exactly at the hp*4 == maxHp boundary (integer-exact)", () => {
  ...
  it("costs 5 MP and sets conviction true once the mid-fight forge has unlocked it", () => {
  ...
  it("persists once active even after healing back above the threshold", () => {
    const s0 = imposterState({ forgeFired: true }, { hero: { hp: 20, maxHp: 130, mp: 16, maxMp: 16 } });
    const s1 = battleReduce(s0, { type: "conv" });
    expect(s1.conviction).toBe(true);
...
describe("the mid-fight Conviction forge", () => {
...
describe("conviction doubles every ability's constant, at reducer level", () => {
  it("PT under both CT and conviction deals 112 through the reducer, not just the helper", () => {
  ...
  it("a DoT already running before Conviction activates keeps ticking 4, not 8, even after activation", () => {
    const s0 = imposterState(
      { forgeFired: true },
...
describe("CLONES pop vs a real hit: wasPop derived at the call site, never inferred from amount===0", () => {
  it("a hit against a non-real slot pops it: wasPop true, zero damage, no mark, no DoT", () => {
  ...
  it("Fan Out vs clones hits all three: both non-real slots pop (wasPop true, zero), the real slot takes 8", () => {
...
describe("Debug during an unfired PULSE charge breaks it, consuming the mark", () => {
...
describe("hero-side mark/DoT from the Imposter's mirrored Debug", () => {
...
describe("DoT anchoring crosses CLONES phase boundaries, at reducer level", () => {
...
describe("Imposter branch coverage in engine.ts", () => {
  ...
  it("attack whiffs against a vanished (untargetable) Imposter: 0 dealt, no MP gain", () => {
...
describe("Imposter boot + engine-generated win line", () => {
  ...
  it("hero arrives at 130/16 with Root Cause in kit (defeatedBosses: alert-storm + cascade + silent-failure)", () => {
    const s = initBattle({
      seed: 42,
      boss: "imposter-syndrome",
      defeatedBosses: ["alert-storm", "cascade", "silent-failure"],
  ...
  describe("the engine-generated win line (runs through the real reducer and asserts observed facts)", () => {
    ...
    it("every action is legal (zero invalid events) and MP-feasible throughout, and no CLONES-phase real-slot hit ever lands before the mark is held", () => {
      const { invalidCount, oracleViolation } = winLine();
      expect(invalidCount).toBe(0);
      expect(oracleViolation).toBe(false);
    });

    it("reaches victory within the 9-14 hero-turn band, entering every phase: clones opened, pulse FIRED, vanish entered and ripped back by Root Cause, mirror fired exactly twice (H7 mirroring Debug, H9 mirroring it again; H11's boss turn never runs, the fight ends first), with the forge crossing and degeneration both reached", () => {
    ...
    it("emits victory + unlock=imposter-syndrome (no post-defeat forge, the rush order ends here), and the final state's defeatedBosses covers RUSH_ORDER", () => {
