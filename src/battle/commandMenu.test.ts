...
describe("deriveMenuView: partition", () => {
...
describe("deriveMenuView: locked teaser", () => {
...
describe("menuReduce: navigation", () => {
...
describe("menuReduce: cursor memory", () => {
...
describe("row cap guard across every valid rush prefix", () => {
...
describe("partition completeness + full-kit reachability", () => {
    ...
    for (const id of ABILITY_ORDER) {
      ...
      expect(
        ...
        `ability id "${id}" belongs to ${count} of {attack, SKILLS_IDS, SPELLS_IDS}, expected exactly 1. ` +
          `If this fires because a NEW ability id was just added to abilities.ts, this is a categorization ` +
  ...
  it("every command in the full kit is reachable as an AbilityRow in exactly one menu level", () => {
